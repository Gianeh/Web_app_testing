...
// function that handles movement inside the map
export function moveTable(event) {
  ...
  // get key or the element id of the button clicked
  ...
  // choose different actions depending on the id
  switch (id) {
    // CASE I MOVE UP
    case "buttonUp":
      if (CurrentOrigin["y"] + Rows + 1 <= MapHeight) {
        // move the table down by one row starting from the last row
        ...
        // modify current origin as we move up
        CurrentOrigin["y"] += 1;
        ...
        for (let j = 0; j < Cols; j++) {
          ...
          // check if there is a player village at the top
    ...
    case "buttonDown":
      if (CurrentOrigin["y"] - 1 >= 0) {
        // move the table up by one row starting from the second row from the bottom
        ...
        // modify current origin as we move down
        CurrentOrigin["y"] -= 1;
        localStorage.setItem("CurrentOrigin", JSON.stringify(CurrentOrigin));
        console.log("CurrentOrigin: " + CurrentOrigin["x"] + " " + CurrentOrigin["y"]);
    ...
    case "buttonLeft":
      if (CurrentOrigin["x"] - 1 >= 0) {
        ...
        // modify current origin as we move left
        CurrentOrigin["x"] -= 1;
        localStorage.setItem("CurrentOrigin", JSON.stringify(CurrentOrigin));
    ...
    case "buttonRight":
      if (CurrentOrigin["x"] + Cols + 1 <= MapWidth) {
        ...
        // modify current origin as we move right
        CurrentOrigin["x"] += 1;
        localStorage.setItem("CurrentOrigin", JSON.stringify(CurrentOrigin));
  ...
  // reset the event listener on the overlay of the enemy villages
