        ...
        for (let j = 0; j < DENSITY; j++) {
            if (rowsWithRocks[j] = i) {
                for (let k = 0; k < rocksPerRow; k++) {
                    ...
                    let rockcell = Math.floor(Math.random() * (table.row[i].cells.length - 1)) + table.row[i].cells.length;
                    ...
                    while (different) {
                        ...
                        for (let x = 0; x < rocksPosition.length; x++) {
                            if (element == rockcell) {
                                rockcell = Math.floor(Math.random() * (table.row[i].cells.length - 1)) + table.row[i].cells.length;
                                break;
                            } else {
                                check++;
                            }
